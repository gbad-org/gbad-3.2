   ...
   while (instanceListNode != NULL)
   {
      ...
      if (instance != NULL)
      {
         ...
         while (otherInstanceListNode != NULL)
         {
            ...
            // we don't need to look at anything before the instance
            // (i.e., instances that already have their frequency set)
            // OR instances that matched previously, as their frequency would
            // already be set due to matching with another instance
            if ((otherInstance != NULL) && (otherInstance->frequency == 0) &&
                (!otherInstance->matched) && (instanceIndex != otherIndex))
            {
               if ((instance->numVertices == otherInstance->numVertices) &&
                   (instance->numEdges == otherInstance->numEdges))
               {
